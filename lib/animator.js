...
var defaultAnimator = {
  ...
  interpolator: function(currentPosition, start, end, velocity, thrown) {
    if(velocity < 0) {
      return interpolators.bounce(currentPosition, start, end, velocity, thrown)
    } else {
      return interpolators.overshoot(currentPosition, start, end, velocity, thrown)
    }
  }
}
...
Animator.prototype.launch = function(velocity, cb, thrown) {
  ...
  if(thrown)
    ...
    this.keyframeAnimate(interpolator.duration, interpolator.fn, done)
}
