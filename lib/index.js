...
function Menu(opts) {
  ...
  if(opts.pullPastDampening) {
    ...
    this.pullPastDampeningClose = opts.pullPastDampening.close || opts.pullPastDampening
  }
  ...
  if(opts.menuSwipeClose) {
    menu.addEventListener('touchstart', menuSwipeClose)
    if(opts.mouseEvents)
      menu.addEventListener('mousedown', menuSwipeClose)

    function menuSwipeClose(evt) {
      var pageOffset = page(that.axis, evt)
        , fingerOffset = pageOffset - that.openPosition
