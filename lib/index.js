...
function Menu(opts) {
  ...
  for(var i = 0 ; i < opts.animators.length ; i++) {
    animator = new Animator(opts.animators[i].els || animatedEls, 0, this.openPosition, this.axis, opts.animators[i])
    this.animators.push(animator)
    if(this.isOpen) {
      animator.updatePosition(this.openPosition)
    }
  }
...
Menu.prototype.start = function(evt) {
  ...
  this.velocity.updatePosition(this.initialOffset)
}
...
Menu.prototype.move = function(evt) {
  ...
  this.velocity.updatePosition(currentPosition)
...
Menu.prototype.end = function() {
  ...
  this.launching = true
  for(var i = 0 ; i < length ; i++) {
    console.log(this.velocity.getVelocity(), this.velocity.positionQueue)
